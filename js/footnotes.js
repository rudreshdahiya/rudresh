renderFootnotes = function () {
  ...
  document.querySelectorAll('.footnotes > ol > li[id^="fn"], #refs > div[id^="ref-"]').forEach(function (fn) {
      ...
      if (/^fn/.test(fn.id)) {
          ...
          side.firstElementChild.innerHTML = '<span>' + number +
      ...
      // change `a` to the last element in the outer article-post div so footnotes show at bottom of page
      // a = document.querySelector('.article-post').lastElementChild;
      insertAfter(a, side);
  ...
  document.querySelectorAll('.footnotes, #refs').forEach(function (fn) {
      ...
      // items[0].style.width = '25%';
      // items[0].style.margin = 'auto';
      // removeEl(items[1]);
      // remove the div containing the HR and OL tags
